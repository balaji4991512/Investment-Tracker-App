...
## 1. Physical Gold Bills

### Challenge: Multiple Formats
Different jewelry stores have different bill formats:
- **Tanishq**: Detailed breakup with item codes
- **Kalyan**: Simple format with combined charges
- **Malabar**: Multiple items per bill
- **Local Shops**: Handwritten or basic computer bills

### Fields to Extract (Universal)
| Field | Variations in Bills | Our Standard Field |
|-------|-------------------|-------------------|
| Company Name | "Tanishq", "TANISHQ JEWELLERY", "Tanishq Ltd" | `vendor.name` |
| Date | "15/01/2025", "15-Jan-25", "January 15, 2025" | `date` (YYYY-MM-DD) |
| Jewelry Type | "Necklace", "NC", "Chain", "Ring", "Coin" | `items[].type` (enum) |
| Weight | "Gross Wt", "GW", "Total Weight" | `items[].weight.gross` |
| Net Weight | "Net Wt", "NW", "Pure Gold" | `items[].weight.net` |
| Purity | "22K", "916", "22 Carat" | `items[].purity.karat` |
| Rate | "Rate/Gm", "Gold Rate", "Price" | `items[].rate` |
| Making Charge | "MC", "Making", "Labour", "VA" | `items[].makingCharge.amount` |
| Wastage | "Wastage", "WC", "Waste %" | `items[].wastage` |
| GST | "CGST", "SGST", "Tax" | `totalAmount.gst` |

### Extraction Strategy

#### Option 1: LLM-Based (Recommended)
Use GPT-4/Claude to understand context:

```dart
// Example prompt for LLM
String extractionPrompt = '''
Extract the following information from this jewelry bill:
...
3. For each item:
   - Type of jewelry
   - Gross weight, net weight, stone weight (in grams)
   - Purity (22K or 24K)
   - Gold rate per gram
   - Making charges
   - Wastage percentage and amount
   - Stone charges if any
4. Total amounts: subtotal, GST breakdown, final amount
...
- Understands context (e.g., "MC" means making charge)
- Can extract from poorly scanned bills
- Works with handwritten bills (if OCR can read it)
...
// Example patterns
final patterns = {
  'tanishq_date': r'Date:\s*(\d{2}[/-]\d{2}[/-]\d{4})',
  'kalyan_weight': r'Gross\s*Wt[:\s]*(\d+\.\d+)\s*gms',
  'rate': r'Rate[/\s]*(?:Gm|Gram)[:\s]*Rs\.?\s*(\d+(?:,\d+)*)',
  'gst': r'CGST[:\s@]*\d+%[:\s]*Rs\.?\s*(\d+(?:\.\d+)?)',
};
```
